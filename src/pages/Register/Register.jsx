    ...
    try {
      if (data.role === "agent") {
        const res = await axios.post(
          `${import.meta.env.VITE_SITE_URL}/agent`,
          save,
        );
        ...
        const res = await axios.post(
          `${import.meta.env.VITE_SITE_URL}/user`,
          save,
        );
        console.log(res.data);
        setErrorMessage("");
    ...
    <div className="bg-zinc-100 min-h-screen flex flex-col items-center w-full">
      ...
      <div className="border mt-[10vh] border-zinc-500 backdrop-blur-md p-3 rounded-md min-w-96">
        ...
        >
          ...
          <Grid item xs={12}>
            <TextField
              ...
              {...register("pin", {
                ...
                pattern: {
                  value: /^\d{5}$/,
                  message: "PIN should have exactly 5 digits",
                },
              })}
              inputRef={pinRef}
            />
          ...
          <button
            className="border border-blue-400 transition-all active:scale-95 hover:scale-105 rounded-sm p-2 uppercase bg-blue-500 text-white"
            type="submit"
          >
            Register
          </button>
        ...
        <Link className="text-sm text-blue-500 inline-block mt-2" to="/login">
          Login
        </Link>
      </div>
    </div>
